...
export default class Batch {
  ...
  constructor(id, cellNumber, type, { warnings, errors } = { warnings: false, errors: false }) {
    this.#id = id;
  ...
  addCell(cell) {
    ...
    if (
      this.#cells.map(addedCell => `${addedCell.x}-${addedCell.y}`).includes(`${cell.x}-${cell.y}`)
    ) {
      allowed = false;
      this.#errors &&
        console.error(
  ...
  getDuplicateValues() {
    return Array.from({ length: this.#maxValue - this.#minValue + 1 }, (_, i) => i + 1).filter(
